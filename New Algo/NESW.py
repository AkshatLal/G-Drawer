...
def num_cips(G):
    ...
    while len(outer_vertices) > 1:
        ...
        outer_vertices.insert(0, cip_store[
            len(cip_store) - 1])  # handles the last vertex of the corner implying path added
        if len(outer_vertices) == 1:  # works for the last vertex left in the boundary
            ...
            for test in cip[0]:  # checks first corner implying path
                if (test, outer_vertices[0]) in H.edges and (test, outer_vertices[0]) not in outer_boundary:
                    ...
                    first_cip = 1
                    break
            if last_cip == 0 and len(
                    cip) != 2:  # if merge is possible as well as both cips are available for last vertex
